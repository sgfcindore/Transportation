...
(function() {
  ...
  window.viewBookingLRDocument = function(id) {
    if (typeof appData !== 'undefined' && appData.booking_lr) {
      var entry = appData.booking_lr.find(function(e) { return e.id === id; });
      if (entry) { showLRViewModal(entry); }
    }
  };

  window.viewNonBookingLRDocument = function(id) {
    if (typeof appData !== 'undefined' && appData.non_booking_lr) {
      var entry = appData.non_booking_lr.find(function(e) { return e.id === id; });
      if (entry) { showLRViewModal(entry); }
    }
  };
...
1. Save this file as: js/lr-document-view.js

2. Add to your dashboard.html (before </body>):
   <script src="js/lr-document-view.js"></script>

3. In your updateBookingLRList() function in dashboard-main.js, 
   change the View button:

   BEFORE:
   <button onclick="viewRecord('booking_lr', '${entry.id}')" ...>👁️ View</button>
   
   AFTER:
   <button onclick="viewBookingLRDocument('${entry.id}')" class="btn-secondary text-xs px-2 py-1">📄 View LR</button>

4. In your updateNonBookingLRList() function, change the View button:
   
   BEFORE:
   <button onclick="viewRecord('non_booking_lr', '${entry.id}')" ...>👁️ View</button>
   
   AFTER:
   <button onclick="viewNonBookingLRDocument('${entry.id}')" class="btn-secondary text-xs px-2 py-1">📄 View LR</button>

5. You can also call showLRViewModal(lrData) directly with any LR data object

The modal will display a professional LR document that matches your physical forms!
*/
